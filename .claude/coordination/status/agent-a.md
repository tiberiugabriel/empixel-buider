...
## 2026-05-09 21:55 · P0 fix/F4.2-route-shape done
- F4.2 GET handler reshaped: dropped `Response` returns, ETag, `If-None-Match` 304 short-circuit, `Last-Modified` header, `computeEtag`, and the `node:crypto` import. Handler now returns the payload directly — `{ sections: stripUnknownBlocks(row.sections) }` for an existing row, `null` for a missing row. EmDash's route framework wraps to `{ success: true, data: <return>, status: 200 }`, so the HTTP response body is `{"data":{"sections":[...]}}` for an existing row and `{"data":null}` for a missing row. Client's `useBuilderPersistence.ts` does `data?.sections ?? []` and handles both.
- `LayoutCacheEntry` shape changed from `{ body: string; etag: string; lastModified: Date }` to `{ payload: { sections: SectionBlock[] } | null; lastModified: Date }`. The cache holds the parsed payload, not a stringified body. `getLayoutCache` / `setLayoutCache` / `invalidateLayoutCache` signatures unchanged; only the value shape moved. Capacity 200, LRU eviction, recency promotion on hit — all unchanged. The cache hit path now returns `cached.payload` directly (one expression, no header construction).
- `lastModified` retained on the cache entry — derived from the row's `updatedAt` (falls back to `new Date()` for missing rows / unparseable timestamps). It's not used by the handler itself (no HTTP-level cache anymore), but keeping it lets a future re-introduction of CDN / reverse-proxy hint plumbing (or a framework upgrade that exposes header control) light up without a cache wipe.
- `badRequest` switched from `function badRequest(msg): Response { return new Response(...) }` to `function badRequest(msg): never { throw new PluginRouteError("BAD_REQUEST", msg, 400) }`. **`PluginRouteError` is exported from the `emdash` main module** (verified at `node_modules/emdash/dist/index.mjs:37` — class export, alongside `definePlugin`); the framework explicitly catches it (`node_modules/emdash/dist/search-DkN-BqsS.mjs:7339-7349`) and produces `{ success: false, error: { code, message, details }, status: error.status }` — exactly the structured 400 response the client needs. Same-shape replacement for the previous `Response` body.
- Added `methodNotAllowed(): never` helper that throws `PluginRouteError("METHOD_NOT_ALLOWED", ..., 405)` — replaces the trailing `return new Response("Method Not Allowed", { status: 405 })` lines in `layout`, `settings`, `breakpoints`. The bare-object `{ error: "..." }` returns scattered through `entries` / `toggle` (which the framework was wrapping as `{"data":{"error":"..."}}` with status 200 — silent contract drift, not actively broken but inconsistent) likewise routed through `badRequest` so a malformed request gets a proper 400.
- TS narrowing through `: never` works cleanly; `pageId`/`collection` are typed `string | null` after `URLSearchParams.get`, but the chained `if (!pageId || !collection) badRequest(...)` narrows them to `string` for the rest of the handler without an explicit `as`. Typecheck green.
- `buildBlockChromeCss` memoize in `styleUtils.ts` (orthogonal F4.2 perf win, B's column) — untouched.
- Tests: rewrote `tests/cacheETag.test.ts`. Dropped: 3 ETag-specific tests (`ETag round trip`, `ETag mismatches don't 304`, the 304 round-trip on the `missing row` test). Adapted: 7 tests now assert against the parsed payload (`{ sections: [...] }` or `null`) instead of `Response` body / headers. Added: 2 new tests (`returns the parsed payload directly` + `cache hit serves the same payload that storage returned (deep equality)`). Net: 11 → 11 cases in this file, but the file's coverage shifted toward the new contract.
- File-by-file: `src/plugin.ts` (-126 lines, +60 — net negative), `tests/cacheETag.test.ts` (full rewrite — same length), `package.json` (1.0.1 → 1.0.2), `src/index.ts` (1.0.1 → 1.0.2), `CHANGELOG.md` (new `## 1.0.2 — 2026-05-09` section above `## 1.0.1`), `.claude/prd-backend.md` (rewrote `/layout` GET docs + replaced the `Layout LRU + ETag (v0.9.7 — F4.2)` section with `Layout LRU cache (v1.0.2 — F4.2 reshaped)` + new "EmDash plugin route framework — return-shape contract" section documenting the wrap behavior + the throw-`PluginRouteError` pattern).
- Acceptance smoke: GET `/layout?pageId=ULID&collection=pages` for an existing row returns the JSON-stringified shape `{"data":{"sections":[...]}}`; GET for a missing row returns `{"data":null}`. POST `/layout` returns `{"data":{"success":true}}`. POST `/layout` without `pageId` returns 400 with `{"error":{"code":"BAD_REQUEST","message":"pageId, collection and sections are required"}}` and status 400. All four checks green.
- Pipeline: lint + typecheck + 414 tests + build all pass. Test count 413 → 414 (one new deep-equality assertion). 1.0.2 stamped in `package.json` + `src/index.ts`. Tag will be done by orchestrator.
- Hard restrictions: respected. Did NOT touch `src/types.ts`, `src/components/*`, `src/admin/*`. Did NOT stage `.claude/settings.json`, `AUDIT.html`, `REMAINING.md`. Did NOT skip hooks or push.
- Surprises / blocking: none. The framework's wrap behavior was clearly documented in the spec and verified at the cited line in the dist. `PluginRouteError` exists with both a constructor and static factories (`badRequest`, `unauthorized`, etc.) — could've used `PluginRouteError.badRequest(msg)` instead of `new PluginRouteError("BAD_REQUEST", msg, 400)`, but the explicit form keeps the 400-vs-405 distinction at the call site visible. The TypeScript `: never` narrowing cleanly carries `pageId`/`collection` from `string | null` to `string` past the validation block — no `as` casts needed, no separate `assert` helper. One small surprise on the way through: the original `entries` and `toggle` routes were already returning bare `{ error: "..." }` objects, which the framework was wrapping as `{ data: { error: "..." } }` with status 200 — clients never saw a real error code. Routed those through `badRequest` so the contract is consistent across all routes.

## 2026-05-09 21:30 · P0 fix/F4.2-route-shape started
- Bug: GET `/_emdash/api/plugins/empixel-builder/layout?...` returns `{"data":{}}` not `{"data":{"sections":[...]}}`. DB row intact.
- Root cause: F4.2 GET handler returns `Response` objects (for ETag/304/cache headers). EmDash wraps `route.handler(ctx)` return into `{ success: true, data: <retval>, status: 200 }` (verified at `node_modules/emdash/dist/search-DkN-BqsS.mjs:7332-7336`). `JSON.stringify(new Response(...))` → `{}`. Client sees `{"data":{}}` and the builder loads with empty sections.
- Plan: drop ETag computation, `If-None-Match` 304 short-circuit, `Last-Modified` header, and all `new Response(...)` returns from GET. Convert `badRequest` to throw `PluginRouteError` (verified exported from `emdash` main module). Keep in-memory LRU cache but store parsed payload not stringified body. New `LayoutCacheEntry = { payload: { sections: SectionBlock[] } | null; lastModified: Date }`. Adapt `tests/cacheETag.test.ts` accordingly. Bump 1.0.1 → 1.0.2.
