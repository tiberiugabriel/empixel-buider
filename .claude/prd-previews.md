...
### Media URL Resolution (1.0.5)
Previews resolving a `storageKey` to a fetchable URL **must** call
`resolveMediaUrl(key)` from `src/components/media.ts` (re-exported via
`empixel-builder/components` for external consumers; admin uses the
relative path `../../components/media.js`). Never hardcode the
`/_emdash/api/media/file/${key}` URL.
...
The helper returns `string | null` — coerce `null` → `undefined` for
React props that expect `undefined` (e.g. `<img src=…>`). Admin
omits the `opts.locals` argument because it runs in browser context;
the helper falls back to the legacy local-runtime URL on local
hosts. A future EmDash admin-side resolver (browser-context analog
of `Astro.locals.emdash.getPublicMediaUrl`) gets routed through the
same call site without touching N preview files again.
...
3. **ImagePreview** — Renders `config.image` via `resolveMediaUrl(image.storageKey)` from `src/components/media.ts` (1.0.5 — was hardcoded `/_emdash/api/media/file/<key>`) with caption + link
...
| 3 | image | Outer is `<figure\|<a>\|<div> data-epx-block id class href? target? rel?>` (figure when caption set; `<a>` when link + no caption; else `<div>`). Inner: `<div class="epx-img-frame">[<a>?]<img src=resolveMediaUrl(…) loading="lazy" decoding="async">[</a>]?</div>{caption ? <figcaption>{caption}</figcaption> : ''}` | `<figure style={{margin:0}}><div class="epx-img-frame" style={justify?}><img src="/_emdash/api/media/file/<key>" style={imgVisualStyle+imgInline}>{caption ? <figcaption>{caption}</figcaption> : ''}</figure>` | yes | Three documented intentional differences: **(a)** preview always uses `<figure>` rather than picking `figure\|a\|div` per caption/link. The frontend root tag carries `data-epx-block`; on canvas the equivalent is the wrapping `<div data-epx-block>` Canvas emits, so the preview's outer `<figure>` is just a margin-zeroed wrapper for the image+caption pair. Dropping the figure would require duplicating the figcaption layout logic. **(b)** preview uses `resolveMediaUrl(key)` from `src/components/media.ts` with no opts (admin runs in browser context, no `Astro.locals`); the helper's legacy fallback emits the same `/_emdash/api/media/file/<key>` URL the literal-versions emitted, so behavior is preserved on local hosts. Frontend uses `resolveMediaUrl(key, { locals })` (F2.2). [Resolved 1.0.5 — admin resolveMediaUrl migration covered all 13 sites in `src/admin/controls/` + `src/admin/previews/`.] **(c)** preview computes `imgVisualStyle` (border / radius / shadow) inline via `buildImgVisualReactStyle`; frontend emits the same declarations via `buildBlockChromeCss({…, imgScoped:true})` scoped to `[data-epx-block="<id>"] img`. Output equivalent at runtime; the preview duplicates the math because Canvas only just (F3.6.3) started emitting the scoped img CSS — the inline-style version is left in for safety until a parity test pins them as equivalent. |
