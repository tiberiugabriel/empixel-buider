...
## E2E coverage (1.0.6)
The right-panel + canvas surface is exercised end-to-end by the Playwright smoke suite added in F4.9 — see [`prd-e2e.md`](prd-e2e.md). The 5 specs cover canvas open, container drag, nested text drag, save+reload persistence, and breakpoint canvas resize. Specs run against a *user-running* EmDash dev server (`npm run test:e2e`); the bundled host fixture is deferred. The suite is **separate** from the default vitest pipeline and is not wired into `prepublishOnly`.
