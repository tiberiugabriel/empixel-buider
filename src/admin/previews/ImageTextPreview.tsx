...
export const ImageTextPreview = memo(function ImageTextPreview({ config }: { config: Record<string, any> }) {
    ...
    <div style={{ ...theme, padding: "14px", display: "flex", gap: 12, flexDirection: isRight ? "row-reverse" : "row", alignItems: "center" }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 4 }}>
          {config.headline || <span style={{ fontStyle: "italic", color: "#bbb" }}>Headline...</span>}
        </div>
        {config.body && <div style={{ fontSize: 11, opacity: 0.7 }}>{config.body.slice(0, 80)}</div>}
      </div>
    </div>
  );
});
